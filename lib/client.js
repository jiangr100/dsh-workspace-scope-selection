  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function ScopeEditor(props) {
        ...
        // projectedRoot (workspace-scope projection root), roots, onClose
        ...
        // failure renders a visible error with a retry.
          ...
          return slots.inject('conversation.input.left', function () {
            return slots.register({
              ...
              inject: function (sessionId) {
                // The session's workspace root never changes; the session
                // list snapshot is the cheapest reliable source (the editor
                // falls back to the projection root and then to
                // /workspace-scope info).
                var root = undefined
                try {
                  var sessions = ctx.get('sessions')
                  if (sessions !== undefined && sessions.list !== undefined && typeof sessions.list.getSnapshot === 'function') {
                    var items = sessions.list.getSnapshot().items
                    if (items !== undefined) {
