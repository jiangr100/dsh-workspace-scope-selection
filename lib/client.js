        ...
        var overlay = React.createElement('div', { className: 'wss-overlay' },
          React.createElement('div', { className: 'wss-modal', ref: modalRef, role: 'dialog', 'aria-label': L('选择可写目录', 'Select writable directories') },
            ...
            React.createElement('div', { className: 'wss-tree' },
              ...
              // The top row is the workspace toggle ONLY at the workspace
              // level; at any other level it is a normal directory row.
              !snap.loading && snap.path !== null && snap.path === snap.root && React.createElement('div', { className: 'wss-row' },
                React.createElement('button', {
                  type: 'button',
              ...
              // Non-workspace levels: the current directory is an ordinary row.
              !snap.loading && snap.path !== null && snap.path !== snap.root && React.createElement('div', { className: 'wss-row' },
                React.createElement('button', {
                  type: 'button',
                  className: 'wss-check' + (coveringRoot(snap.path, snap.draft.roots) !== undefined ? ' wss-checkOn' : ''),
                  disabled: snap.saving || (coveringRoot(snap.path, snap.draft.roots) !== undefined && snap.draft.roots.indexOf(snap.path) === -1),
                  'aria-label': L('切换目录', 'Toggle directory') + ' ' + snap.path,
                  title: coveringRoot(snap.path, snap.draft.roots) !== undefined && snap.draft.roots.indexOf(snap.path) === -1
                ...
                }, coveringRoot(snap.path, snap.draft.roots) !== undefined ? IconCheck() : null),
                React.createElement('span', { className: 'wss-rowName' },
                  IconFolder(),
                  React.createElement('span', { style: { minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, snap.path),
                ),
                coveringRoot(snap.path, snap.draft.roots) !== undefined && snap.draft.roots.indexOf(snap.path) === -1 &&
