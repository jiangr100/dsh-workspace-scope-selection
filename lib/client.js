...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Normalize the projected selection the same way the host fold does: the
    // workspace root is a member of the writable set exactly when the
    // workspace is writable. This also self-corrects a stale projection
    // value whose roots predate the workspace-membership model — the
    // `workspace` marker is still valid, so the workspace root is re-added.
    function normalizeDraftRoots(roots, workspaceRoot, workspace) {
      var list = Array.isArray(roots) ? roots.filter(function (root) { return typeof root === 'string' }) : []
      if (workspace === false) {
        return list.filter(function (root) { return root !== workspaceRoot })
      }
      return workspaceRoot !== undefined && workspaceRoot !== null && workspaceRoot !== '' && list.indexOf(workspaceRoot) === -1
        ? [workspaceRoot].concat(list)
        : list
    }
    ...
    function apply(ctx) {
      ...
      function ScopeEditor(props) {
        ...
        // projectedRoot (workspace-scope projection root), scopeRoots (the
        // projection's selection), scopeWorkspaceRoot (projection workspace
        // root), scopeWorkspace (projection workspace marker), onClose
        var state = React.useState({
          ...
          retryToken: 0,
          // Local pending selection: the writable directories, the session
          // workspace root included exactly when it should stay writable.
          // Toggles edit this draft immediately; Done persists it once.
          draft: normalizeDraftRoots(props.scopeRoots, props.scopeWorkspaceRoot, props.scopeWorkspace),
        })
        var snap = state[0]
        var setSnap = state[1]
        ...
        // failure or timeout renders a visible error with a retry. When no
        // selection was ever recorded, the default draft is the workspace
        // itself (writable).
        function applyRoot(root, source) {
          var freshDefault = snap.draft.length === 0 &&
            (props.scopeWorkspaceRoot === undefined || props.scopeWorkspaceRoot === null || props.scopeWorkspaceRoot === '')
          patch(Object.assign(
            { root: root, rootSource: source, phase: L('正在加载目录…', 'Loading directories…') },
            freshDefault ? { draft: [root] } : {},
          ))
        }
        ...
        function save(draft) {
          ...
          runCommand(props.sessionId, '/workspace-scope set ' + JSON.stringify(draft)).then(function (outcome) {
            if (settled) return
            settled = true
            clearTimeout(timer)
            if (!outcome.ok) {
        ...
        // matches the writable set. A row that is only COVERED by a selected
        ...
        var overlay = React.createElement('div', { className: 'wss-overlay' },
          React.createElement('div', { className: 'wss-modal', ref: modalRef, role: 'dialog', 'aria-label': L('选择可写目录', 'Select writable directories') },
            ...
            React.createElement('div', { className: 'wss-caption' },
              L('勾选的目录及其子目录允许 agent 写入，勾选状态就是实际的可写范围。顶部的当前目录是工作区：默认已勾选（可写），取消勾选它会使工作区变为只读。取消勾选父目录会移除整棵子树。', 'Checked directories and everything beneath them are writable by the agent — the checked state IS the writable scope. The current directory at the top is the session workspace: checked (writable) by default, unchecking it makes the workspace read-only. Unchecking a parent removes its whole subtree.'),
            ),
            ...
            React.createElement('div', { className: 'wss-tree' },
              ...
              // The current directory is an ordinary row at every level —
              // at the workspace level it IS the workspace, so its checkbox
              // shows exactly whether the workspace is in the selection.
              !snap.loading && snap.path !== null && React.createElement('div', { className: 'wss-row' },
                React.createElement('button', {
                  ...
                  title: coveringRoot(snap.path, snap.draft) !== undefined && snap.draft.indexOf(snap.path) === -1
                    ? L('经父目录包含：取消父目录后整个子树将不再可写', 'Included via a parent directory; uncheck the parent to remove its whole subtree')
                    : snap.path === snap.root
                ...
                snap.path === snap.root && coveringRoot(snap.path, snap.draft) === undefined &&
                  React.createElement('span', { className: 'wss-hint' }, L('工作区只读', 'workspace read-only')),
              ),
            ...
            React.createElement('div', { className: 'wss-foot' },
              React.createElement('span', { className: 'wss-footRoots', title: snap.draft.join('\n') },
                (snap.root !== null && snap.draft.indexOf(snap.root) !== -1
              ...
              React.createElement('button', {
                ...
                onClick: function () { patch({ draft: [], error: null }) },
              }, L('清除全部', 'Clear all')),
              React.createElement('button', {
                type: 'button',
      ...
      function ScopeButton(props) {
        ...
        var workspaceSelected = projectedRoot !== undefined && roots.indexOf(projectedRoot) !== -1
        var title = workspaceSelected
          ? L('编辑可写目录范围（已选择 ' + String(roots.length) + ' 个目录，含工作区）', 'Edit the writable directory scope (' + String(roots.length) + ' selected, workspace included)')
          : L('编辑可写目录范围（已选择 ' + String(roots.length) + ' 个目录，工作区只读）', 'Edit the writable directory scope (' + String(roots.length) + ' selected, workspace read-only)')
        return React.createElement(React.Fragment, null,
          ...
          open && React.createElement(ScopeEditor, {
            ...
            scopeRoots: roots,
            scopeWorkspaceRoot: projectedRoot,
            scopeWorkspace: scope !== undefined && scope.workspace !== false,
