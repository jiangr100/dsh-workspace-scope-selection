  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      function ScopeEditor(props) {
        ...
        // projection's normalized selection — the workspace root included
        // when writable), scopeWorkspaceRoot (projection workspace root;
        // empty when no selection was ever recorded), onClose
        var state = React.useState({
          ...
          draft: Array.isArray(props.scopeRoots) ? props.scopeRoots.slice() : [],
        })
        var snap = state[0]
        var setSnap = state[1]
