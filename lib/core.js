 ...
 * a plain last-wins scan.
 * @param events - session events in log order.
 * @returns `{ roots, workspace }` — the selected roots (canonical absolute
 *   paths) and whether the session workspace itself stays writable
 *   (defaults to true; an absent field means a legacy event, which is
 *   workspace-writable).
 */
export function selectionOf(events) {
 ...
 */
export function augmentConfinedArgv(confined, extraRoots, tmpRoots = []) {
 ...
 * @param policy - the resolved policy (mode + workspaceRoot +
 *   extraWritableRoots + optional workspaceWritable flag).
...
export function renderSelectedPolicyText(policy) {
  ...
  const workspaceWritable = policy.workspaceWritable !== false;
 ...
 */
export function tempWritableRoots() {
