...
function patchResolve(service) {
  ...
  service.resolve = (request = {}) => {
    ...
    if (session === void 0) {
      return { ...resolved, extraWritableRoots: [], workspaceWritable: true };
    }
    const selection = selectionOf(session.events);
    // When the workspace is excluded, a legacy roots entry naming the
    // workspace root itself must not re-grant it.
    const roots = selection.workspace
      ? selection.roots
      : selection.roots.filter((root) => root !== resolved.workspaceRoot);
    return {
      ...resolved,
      extraWritableRoots: roots,
      workspaceWritable: selection.workspace,
    };
  };
  return () => {
...
function patchFsFence(fs, resolvePolicy) {
  ...
  fs.checkedTarget = async (target, sandboxPolicy) => {
    ...
    const roots = policy.workspaceWritable === false
      ? tempWritableRoots()
      : workspaceWritableRoots(policy.workspaceRoot);
 ...
 * splice the extra-root grants into the selected runner's dialect. When the
 * workspace itself is excluded from the selection, the base is a `read-only`
 * translation and the platform temp areas plus the selected roots are
 * spliced in, so the workspace root is genuinely read-only. Other modes
...
function patchConfine(provider) {
  ...
  provider.confine = (argv, policy) => {
    ...
    if (policy.workspaceWritable === false) {
...
async function handleWorkspaceScope(invocation, ctx) {
  ...
  const appendSelection = (nextRoots, workspace = true) => {
  ...
  switch (verb) {
    case "":
      return {
        kind: "success",
        text: `workspace-scope: ${roots.length} selected root(s), workspace ${selection.workspace ? "writable" : "read-only"}; verbs: set <json> | clear | info | list <path>`,
      };
    case "set": {
      let parsed;
      try {
        ...
        return { kind: "error", text: "workspace-scope: \"set\" expects a JSON object {roots, workspace} or a JSON array of absolute directory paths" };
      }
      try {
        // Object form: { roots: string[], workspace?: boolean } — the whole
        // selection. Legacy array form: roots only, workspace stays writable.
        let rootInput;
        let workspace;
        if (Array.isArray(parsed)) {
          ...
          rootInput = parsed.roots;
          workspace = parsed.workspace !== false;
        } else {
          return { kind: "error", text: "workspace-scope: \"set\" expects a JSON object {roots, workspace} or a JSON array of absolute directory paths" };
        }
        const normalized = normalizeRoots(rootInput, canonicalPath);
        appendSelection(normalized, workspace);
        return {
          kind: "success",
          text: `workspace-scope: ${normalized.length} root(s) selected, workspace ${workspace ? "writable" : "read-only"}`,
        };
      } catch (error) {
    ...
    case "info":
      return {
        kind: "success",
        text: JSON.stringify({ workspaceRoot, roots, workspace: selection.workspace }),
      };
    case "list": {
      ...
      apply: (state, event) =>
        event.type === SELECTION_EVENT
          ? {
              ...
              workspace: event.data?.workspace !== false,
            }
          : state,
      view: (state) => state,
      stateVersion: 1,
